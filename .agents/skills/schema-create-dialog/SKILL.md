...
## Goal
Build a create flow that respects schema authority routing, generated endpoints, and post-create sync behavior.
...
- explicit Typesense sync behavior after create

## Rules
- Use `useApiProcess()` / `useCRUD()` for the actual mutation.
...
- If the model is Typesense-enabled, perform explicit post-create sync instead of assuming the directory refresh is enough.
- Keep the UI component focused on capture and feedback; keep orchestration in composables, stores, or page-level handlers.

## Definition of done
- The create form sends the correct payload to the generated endpoint.
- Authority routing is correct for the model.
