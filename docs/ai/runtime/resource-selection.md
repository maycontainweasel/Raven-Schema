...
3. Confirm the exact field shape in the generated router or runtime template before wiring the UI.
4. If no existing contract fits, inspect `config/graph.mpdg` and the matching `config/specs/**` files to see whether a new resource or function-backed view is the right schema change.
...
- New reusable resources belong in schema authoring inputs, not only in app page code.
