...
Depends on: docs/schema-kit-overrides.md, docs/module-overlays.md
...
- App config supports `documentation.schemaApp` which injects SchemaDocs as a project and emits routers/types into it
...
## Overrides (schema‑kit)

Schema‑kit supports structured overrides in the target app so teams can extend or replace module behavior without editing the module:

- Controllers: `schema/controllers/<model>.ts`
- Stores: `schema/stores/auth.ts`, `schema/stores/apiModels.ts`
- Models manifest: `schema/models.override.ts`
- Runtime overrides (advanced): `schema/overrides/schema-kit/**`
- Module‑source resources + overrides:
