...
import { scaffoldCreateRecordOverride } from '../../../../utils/model-override-manager'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const result = await scaffoldCreateRecordOverride(model.modelKey, { force })
    return {
      ok: true,
      model,
      override: result,
    ...
    throw createError({
      statusCode: 500,
      statusMessage: error?.message ?? 'Failed to scaffold create record override.',
    })
  }
})
