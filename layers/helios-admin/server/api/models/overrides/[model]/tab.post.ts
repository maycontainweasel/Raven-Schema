...
import { scaffoldTabOverride } from '../../../../utils/model-override-manager'
...
export default defineEventHandler(async (event) => {
  ...
  const tabSlug = String(body?.tabSlug ?? body?.slug ?? '').trim()
  const force = Boolean(body?.force)
  if (!tabSlug) {
    throw createError({
      statusCode: 400,
      statusMessage: 'tabSlug is required.',
    })
  }

  try {
    const result = await scaffoldTabOverride(model.modelKey, tabSlug, { force })
    return {
      ok: true,
      model,
      override: result,
    ...
    throw createError({
      statusCode: 500,
      statusMessage: error?.message ?? 'Failed to scaffold tab override.',
    })
  }
})
