...
Depends on: docs/primer/README.md

## Why this matters

SurrealDB record IDs are powerful but easy to misuse. The ID can be *composite* (arrays/objects built from other IDs), and the API surface expects a **specific** representation depending on where you are in the stack. This primer defines the exact terms and rules we use everywhere.
...
1) **Frontend TRPC endpoints expect sub-IDs**
   ...
   - Example: `id: "abc123"` (not `{ tb: "user", id: "abc123" }`).
...
**Rule:** Any snippet we generate must import types from `@schema/types` rather than redefining them.
...
## Generator expectations (ID rules)

1) If `id` is omitted in a graph table, SurrealDB defaults to auto‑IDs.
2) If `id: $field` is used, that field **must exist** in the fields list and be **required**.
3) If a field is required for ID construction, it must be marked with `!` in the graph.

These rules are enforced at generation time to prevent mismatched ID typing.
