...
import type { ApiToolkitConsoleEntry } from '~/stores/apiToolkitConsole'

const props = defineProps<{
  entries: ApiToolkitConsoleEntry[]
  autoClear: boolean
  autoScroll: boolean
}>()
...
const badgeClass = (state?: ApiToolkitConsoleEntry['state']) => {
      ...
      >
        ...
        <div v-else class="mt-2 space-y-2">
          <div class="flex items-center gap-2">
            <span class="badge" :class="badgeClass(entry.state)">
              {{ entry.state ?? 'info' }}
            </span>
