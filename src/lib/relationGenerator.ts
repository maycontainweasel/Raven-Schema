...
function buildAttachFunction(relation: NormalizedRelation): string {
  ...
  return [
    ...
    `\t\t};`,
    '',
    `\t\tlet $REL = select * from ${relation.edge} where in = $REL_ID and out = $RIGHT_ID;`,
    `\t\tif !$REL {`,
    `\t\t\tlet $EDGE_ID = type::record("${relation.edge}", fn::stringID([$REL_ID, $RIGHT_ID]));`,
    `\t\t\trelate only $REL_ID->${relation.edge}->$RIGHT_ID set id = $EDGE_ID;`,
    `\t\t};`,
    `\t};`,
    '',
    `\treturn $REL_IDS;`,
...
function buildDetachFunction(relation: NormalizedRelation): string {
  ...
  return [
    ...
    '',
    `\tfor $REL_ID in $REL_IDS {`,
    `\t\tdelete from ${relation.edge} where in = $REL_ID and out = $RIGHT_ID;`,
    `\t};`,
    `\treturn $REL_IDS;`,
    `};`,
    '',
